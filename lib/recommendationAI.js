 ...
 * recommendationAI.js
 *
 * Generates personalized book recommendations using openai/gpt-oss-120b via Groq.
 *
 * Architecture:
 * - Receives an array of recognized books (title, author, confidence, categories, description)
 * - Optionally receives user preferences (genres, authors, language, reading level)
 ...
 * - Sends the prompt to Groq (OpenAI-compatible chat completions API) with a
 *   strict JSON schema so the response shape is enforced by constrained decoding
...
// gpt-oss-120b replaced llama-4-scout (shut down 2026-07-17). Chosen over
// qwen3.6-27b for the text path because it is Production tier rather than
// Preview, costs less, and is the only option supporting strict json_schema.
const MODEL_ID = "openai/gpt-oss-120b";
// Friendly label for metadata.model_used, matching the vision path.
const MODEL_LABEL = "GPT-OSS 120B";
/**
 * Maximum time to wait for the LLM response in milliseconds.
 * gpt-oss-120b is a reasoning model, so it emits reasoning tokens before the
 * answer. Measured ~1.3s for this prompt at reasoning_effort "low", but the
 * ceiling allows for queueing under load.
 */
 ...
 *
 * This was 1024 under llama-4-scout, which was NOT a reasoning model. On
 * gpt-oss-120b reasoning tokens are charged against this same budget (they show
 * up under usage.completion_tokens_details.reasoning_tokens), so a budget sized
 * for the answer alone can be consumed by reasoning and return empty or
 * truncated content. 4096 leaves ample headroom for both.
 */
const MAX_TOKENS = 4096;

/**
 * Strict JSON schema for the response. Strict mode uses constrained decoding, so
 * the model physically cannot emit a shape that violates this.
 *
 * Groq's strict mode requires every property to be listed in `required` and
 * `additionalProperties: false` at each level. It also cannot enforce a
 * root-level array, hence the {"recommendations": [...]} wrapper — which
 * parseRecommendations() Strategy 4 already knows how to unwrap.
 */
  ...
  try {
    const response = await fetch(GROQ_API_URL, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.7,
        /**
         * gpt-oss reasoning always lands in a separate message.reasoning field,
         * so message.content stays clean JSON — no <think> stripping needed.
         * "low" is right for this task: measured 23 reasoning tokens, and the
         * schema does the structural work that reasoning would otherwise do.
         *
         * Do NOT add reasoning_format here — gpt-oss models reject it. That
         * parameter is qwen/minimax only.
         */
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Groq API returned ${response.status}: ${errorBody}`,
      );
    }
    ...
    if (error.name === "AbortError") {
      ...
      throw new Error(
        `Groq recommendation generation timed out after ${REQUEST_TIMEOUT_MS}ms`,
      );
    }
 ...
 * - gpt-oss-120b follows system instructions carefully
...
7. Respond ONLY with a valid JSON object. No extra text, no markdown, no explanations outside the JSON.
...
function parseRecommendations(rawResponse) {
  ...
  // Expected path. The strict json_schema guarantees {"recommendations": [...]},
  // so that shape is checked first — leaving it to the greedy regex in Strategy 2
  // would risk a stray ']' inside a reason string truncating the match.
  // The bare-array branch is kept for responses produced before the schema
  // existed, and for any model that ignores the wrapper.
  try {
    const parsed = JSON.parse(rawResponse.trim());
    if (Array.isArray(parsed.recommendations) && parsed.recommendations.length > 0) {
      console.log(
        "[recommendationAI] Parsed via Strategy 1: schema-shaped JSON object",
      );
      return validateAndClean(parsed.recommendations);
    }
