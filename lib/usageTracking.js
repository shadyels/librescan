 ...
 * three external APIs:
 *   - qwen3.6-27b vision (book recognition)  → column: groq_vision_requests
 *   - gpt-oss-120b text (recommendations)    → column: groq_text_requests
...
//   - Groq free tier: 1,000 requests/day per model (RPD), so this matches
...
// membership against.
const VALID_API_NAMES = new Set(['groq_vision', 'groq_text', 'google_books'])
 ...
 *
 * Args:
 *   apiName (string): One of 'groq_vision', 'groq_text', or 'google_books'.
...
export async function checkLimit(apiName) {
  ...
  // Example: apiName='groq_vision' → column='groq_vision_requests'
  //
  // This is safe because we validated apiName against VALID_API_NAMES above.
  // The resulting column name is always one of:
  //   'groq_vision_requests', 'groq_text_requests', 'google_books_requests'
 ...
 *     Shape: { date, groq_vision_requests, groq_text_requests, google_books_requests,
