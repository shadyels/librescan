 ...
 * - saved: tracks whether the current recommendation set is saved (Phase 5)
 * - saving: true while PATCH /api/saved is in flight (Phase 5)
 * - deleting: true while DELETE /api/saved is in flight (Phase 5)
 * - showDeleteDialog: controls the confirmation dialog visibility (Phase 5)
 *
 * Phase 5 additions:
 * - Save button (when saved=false): calls PATCH /api/saved to set saved=TRUE
 * - Remove button (when saved=true): opens confirmation dialog, then calls
 *   DELETE /api/saved to permanently delete the scan + recommendations,
 *   and redirects to /saved
 * - Confirmation dialog: warns that deletion is permanent, requires explicit
 *   confirmation before proceeding
 *
 * Dependencies:
 * - react-router-dom: useParams (scanId), useNavigate (back button, delete redirect)
 * - SessionContext: deviceId for API auth (POST, PATCH, DELETE)
...
export default function Recommendations() {
  ...
  // ---- Phase 5: Save/Remove state ----
  // saved: tracks whether this recommendation set is saved in the DB.
  // null initially because we don't know until the GET response tells us.
  // Set to getData.saved on Phase 1 success, or false after Phase 2 (just generated).
  const [saved, setSaved] = useState(null);
  // saving: true while PATCH /api/saved is in flight. Disables the save button
  // and shows a spinner to prevent double-clicks.
  const [saving, setSaving] = useState(false);
  // deleting: true while DELETE /api/saved is in flight. Disables the confirm
  // button in the dialog to prevent double-clicks.
  const [deleting, setDeleting] = useState(false);
  // showDeleteDialog: controls visibility of the confirmation dialog.
  // true when user clicks "Remove from Saved", false on cancel or after delete.
  ...
  useEffect(() => {
    ...
    async function loadRecommendations() {
      try {
        ...
        console.log(
          `[Recommendations] Checking for existing recommendations for scan ${scanId}`,
        );
        const getResponse = await fetch(`/api/recommendations/${scanId}`);

        if (getResponse.ok) {
          // Recommendations already exist in the database.
          // Display them immediately — no LLM call needed.
          const getData = await getResponse.json();
          console.log("[Recommendations] Found existing recommendations");
          setRecommendations(getData.recommendations);
          // Phase 5: extract saved status from API response.
          // This determines which button to show (save vs remove).
        ...
        if (getResponse.status !== 404) {
          // Unexpected error (500, etc.) — don't proceed to generation
          const errorData = await getResponse.json().catch(() => ({}));
          throw new Error(
            errorData.error || `Server error: ${getResponse.status}`,
          );
        }

        // ---- Phase 2: Generate new recommendations ----
        // GET returned 404 — no recommendations exist yet.
        // Switch from "loading" to "generating" state for different UI messaging.
        console.log(
          "[Recommendations] No existing recommendations, generating...",
        );
        setLoading(false);
        setGenerating(true);
        ...
        if (!postResponse.ok) {
          ...
          throw new Error(
            errorData.error || `Generation failed: ${postResponse.status}`,
          );
        }
        ...
        setGenerating(false);
        // Phase 5: newly generated recommendations are always unsaved.
        // The DB default is saved=FALSE. Set explicitly so the save button appears.
        setSaved(false);
      } catch (err) {
  ...
  // Sends PATCH /api/saved to mark this recommendation set as saved (saved=TRUE).
  // On success: flips the button from "Save" to "Remove from Saved".
  // On error: logs it but doesn't break the page — recommendations are still visible.
  const handleSave = async () => {
    ...
    try {
      ...
      console.log(
        `[Recommendations] Saved recommendation set for scan ${scanId}`,
      );
      setSaved(true);
    } catch (err) {
      console.error(`[Recommendations] Save error: ${err.message}`);
    } finally {
      setSaving(false);
    }
  };

  // ---- Phase 5: Delete handler ----
  // Called after user confirms in the dialog. Sends DELETE /api/saved to permanently
  // remove the scan and its cascade-deleted recommendations from the DB.
  // On success: redirects to /saved (the data no longer exists at this URL).
  // On error: closes dialog, logs error. Data is still intact so user can retry.
  //
  // Why [scanId] array: The DELETE endpoint accepts an array of scan_ids to support
  // both single deletion (here) and bulk deletion (Saved page). Wrapping scanId in
  // an array keeps the API contract consistent between both callers.
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log(
        `[Recommendations] Deleted scan ${scanId} and its recommendations`,
      );
      // Navigate to /saved. The scan no longer exists, so this URL would 404
      // if the user stayed here. Redirect is the correct UX.
      navigate("/saved");
    } catch (err) {
      console.error(`[Recommendations] Delete error: ${err.message}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {generating && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <div className="relative">
              {/* Outer ring: spinning blue circle */}
        ...
        {!loading && !generating && !error && books.length > 0 && (
          <>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {metadata.processing_time_ms && (
                <p className="text-gray-400 text-xs">
                  Generated in {(metadata.processing_time_ms / 1000).toFixed(1)}
                  s
                </p>
              )}
            </div>
            ...
            {/* ---- Phase 5: Save/Remove button + Back button ---- */}
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              {/* Save/Remove button: shown only when saved state is known (not null) */}
              {saved === false && (
                <button
                  onClick={handleSave}
                  disabled={saving}
                ...
                >
                  {saving ? (
                    <>
                      {/* Inline spinner: small spinning circle inside the button */}
                      <svg
                    ...
                    <>
                      {/* Bookmark icon: represents saving/collecting */}
                      <svg
                        className="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
              ...
              {saved === true && (
                ...
                >
                  {/* Trash icon: signals destructive action */}
                  <svg
                    className="w-4 h-4"
                    fill="none"
                    stroke="currentColor"
      ...
      {/* Rendered outside the max-w-4xl container so the backdrop covers the full viewport.
          Only mounted when showDeleteDialog is true (no hidden DOM when closed). */}
      {showDeleteDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          {/* Backdrop: semi-transparent black. Clicking it cancels (same as Cancel button). */}
          <div
            className="absolute inset-0 bg-black/50"
            onClick={() => !deleting && setShowDeleteDialog(false)}
          />

          {/* Dialog card: centered, white, with padding and rounded corners. */}
          <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            ...
            {/* Warning text: explains what will be lost */}
            ...
            <div className="flex gap-3 mt-6">
              {/* Cancel: gray, closes dialog. Disabled while delete is in flight
                  to prevent the user from dismissing while the request is pending. */}
              <button
              ...
              {/* Confirm: red, calls handleDelete. Shows spinner while in flight. */}
              <button
